...
[`crate::secrets`](https://github.com/mbryantms/folio/blob/main/crates/server/src/secrets.rs). Backup the
`secrets/` directory.
...
- ~~**OTLP exporter** (`COMIC_OTLP_ENDPOINT`)~~: **considered, not chosen
  ...
  a user reports a need Prometheus can't cover.
...
1. Add a `SettingDef` to the
   [`REGISTRY`](https://github.com/mbryantms/folio/blob/main/crates/server/src/settings/registry.rs) const.
   Mark `is_secret: true` if the value should be sealed before storage
   and redacted in responses.
2. Add a match arm in
   [`apply_overlay_row`](https://github.com/mbryantms/folio/blob/main/crates/server/src/config.rs) that binds
...
3. If the new key triggers a live side-effect (rebuild email,
   ...
   [`admin_settings::update`](https://github.com/mbryantms/folio/blob/main/crates/server/src/api/admin_settings.rs)
...
5. Wire a bootstrap seeder in
   [`settings/bootstrap.rs`](https://github.com/mbryantms/folio/blob/main/crates/server/src/settings/bootstrap.rs)
