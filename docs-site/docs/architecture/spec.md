...
This is the long-form design document. It predates parts of the
current implementation — §9 (Sync) describes the dropped Automerge
plan with a decision note explaining the simpler model that shipped
instead; other "Phase N" callouts may have shipped, been deferred,
or been dropped. When in doubt, treat the live code and the rest of
this docs site as the source of truth.
...
| Web | Next.js 16 (App Router) + React 19 + TypeScript | RSC for data-heavy pages, mature ecosystem, full library coverage. Self-hosted via `output: 'standalone'` in distroless container — **not** Vercel-deployed. |
...
| Sync | `progress_records` with server-side `max(last_page)` conflict resolution | Automerge / CRDT was considered and dropped 2026-05-15 — see §9. The API contract is stable; if a CRDT backend ever ships, the swap is server-internal. |
...
[Library scanner reference](./library-scanner) §4.2 for the full
...
### 6.8 Typo tolerance
- Two-stage: full-text query first (fast, exact). If fewer than 3 results, run a `pg_trgm` similarity query as fallback and merge.
