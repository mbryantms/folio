...
- [`crates/server/src/api/issue_ocr.rs`](https://github.com/mbryantms/folio/blob/main/crates/server/src/api/issue_ocr.rs)
  — request shape, ACL, decode, cache plumbing.
- [`crates/server/src/ocr/pipeline.rs`](https://github.com/mbryantms/folio/blob/main/crates/server/src/ocr/pipeline.rs)
  — `detect → snap → crop → recognize`, all inside one
  `spawn_blocking` so the reactor isn't stalled.
- [`crates/server/src/ocr/detector.rs`](https://github.com/mbryantms/folio/blob/main/crates/server/src/ocr/detector.rs)
  — `comic-text-detector` singleton (`tokio::sync::OnceCell` +
  `std::sync::Mutex<ComicTextDetector>`).
- [`crates/server/src/ocr/recognizer/western.rs`](https://github.com/mbryantms/folio/blob/main/crates/server/src/ocr/recognizer/western.rs)
  — Tesseract via `tesseract-rs`, English-only (`tessdata_best/eng`).
- [`crates/server/src/ocr/recognizer/manga.rs`](https://github.com/mbryantms/folio/blob/main/crates/server/src/ocr/recognizer/manga.rs)
  — `manga-ocr` (encoder + decoder + vocab ONNX, greedy decode).
- [`crates/server/src/ocr/cache.rs`](https://github.com/mbryantms/folio/blob/main/crates/server/src/ocr/cache.rs)
  — Redis-backed result cache.
- [`crates/server/src/api/admin_ocr.rs`](https://github.com/mbryantms/folio/blob/main/crates/server/src/api/admin_ocr.rs)
...
[`crates/server/src/middleware/rate_limit.rs`](https://github.com/mbryantms/folio/blob/main/crates/server/src/middleware/rate_limit.rs).
...
- `/admin/server` UI tile — pairs the JSON with the build /
  ...
  [`web/components/admin/observability/ServerInfoClient.tsx`](https://github.com/mbryantms/folio/blob/main/web/components/admin/observability/ServerInfoClient.tsx).

## Web client

[`web/app/[locale]/read/[seriesSlug]/[issueSlug]/marker-selection.ts`](https://github.com/mbryantms/folio/blob/main/web/app/%5Blocale%5D/read/%5BseriesSlug%5D/%5BissueSlug%5D/marker-selection.ts)
...
- Server validation / ACL / archive / decode / region bounds /
  pipeline-reach: [`crates/server/tests/issue_ocr.rs`](https://github.com/mbryantms/folio/blob/main/crates/server/tests/issue_ocr.rs).
- Cache hit / miss / lang-scoped key:
  [`crates/server/tests/issue_ocr.rs`](https://github.com/mbryantms/folio/blob/main/crates/server/tests/issue_ocr.rs)
  (M4 section).
- Admin endpoint + RequireAdmin:
  [`crates/server/tests/admin_ocr.rs`](https://github.com/mbryantms/folio/blob/main/crates/server/tests/admin_ocr.rs).
- Recognizer trait + heavy E2E (ignored by default):
  [`crates/server/tests/ocr_recognizer.rs`](https://github.com/mbryantms/folio/blob/main/crates/server/tests/ocr_recognizer.rs).
  Run with `cargo test -p server --test ocr_recognizer -- --ignored`.
- Web client fetch shape + fallback paths:
  [`web/tests/reader/marker-selection.test.ts`](https://github.com/mbryantms/folio/blob/main/web/tests/reader/marker-selection.test.ts).
