...
- [`comic-reader-spec.md`](https://github.com/mbryantms/folio/blob/main/comic-reader-spec.md) — overall product
  spec; the scanner is §4 (lifecycle), §6 (parsing), §7 (identity), §10
  (health issues), §14 (operations).
- [`docs/dev/phase-status.md`](https://github.com/mbryantms/folio/blob/main/docs/dev/phase-status.md) — what shipped when (snapshot; consult `git log` for the current state).
...
|---|---|---|
| Manual library scan | `POST /libraries/{slug}/scan` ([api/libraries.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/api/libraries.rs)) | 202 + `{ scan_id, state, coalesced, mode, coalesced_into, queued_followup, reason }`. `mode=normal | metadata_refresh | content_verify`; `?force=true` remains a content-verify alias. Coalesces on an existing in-flight scan. |
| Manual series scan | `POST /series/{slug}/scan` | Per-folder rescan via [`jobs/scan_series.rs`](https://github.com/mbryantms/folio/blob/main/crates/server/src/jobs/scan_series.rs) with `JobKind::Series`. Manual clicks set `force=true`. |
| Manual issue scan | `POST /series/{series_slug}/issues/{issue_slug}/scan` | Same job type, `JobKind::Issue` — runs [`scan_issue_file`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs). |
| Scheduled scan | `library.scan_schedule_cron` per-library | [`jobs/scheduler.rs`](https://github.com/mbryantms/folio/blob/main/crates/server/src/jobs/scheduler.rs) — `coalesce_scan(.., false)`. 5- or 6-field cron via `tokio_cron_scheduler`. |
...
[mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs)) runs
five phases. Series and issue paths reuse phases 3–5 in a narrowed form.

### 4.1 Validate

- **Where**: [`scanner/validate.rs`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/validate.rs)
...
- **Output**: `Result<(), ValidationError>`. The narrow series path uses
  the lighter [`folder_still_exists`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/validate.rs)
  check instead.

### 4.2 Enumerate

- **Where**: [`scanner/enumerate.rs`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/enumerate.rs)
  (called from [mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs) on a
  blocking thread).
...
[`enumerate::is_series_subfolder_name`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/enumerate.rs).
...
- **Where**: [`build_library_scan_plan`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs).
- **What**: for each series-folder candidate, decide whether to walk it
  this scan, and if so list every recognized archive inside.
- **Fast-path branch** ([mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs)):
  when `force=false` and the series row carries a `last_scanned_at`,
  call [`list_archives_changed_since`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/enumerate.rs)
...
- **Where**: [`process_planned_folder`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs),
  fanned out from [mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs)
...
- **Steps** in order:
  ...
  1. **Skip-unchanged short-circuit**
     ([mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs)).
     ...
     `processed=false`.
  2. **Series identity** ([mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs)).
     ...
     Then call [`identity::resolve_or_create`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/identity.rs):
     ...
     Then emit [`ScanEvent::SeriesUpdated`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs)
  ...
  3. **Per-archive ingest loop**
     ...
     [`ingest_one_with_fingerprint`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs),
     which handles:
     - extension-based skip for `.cbr` / `.cb7` →
       `UnsupportedArchiveFormat`
       ([process.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs))
     ...
     - DuplicateContent detection (hash collision under a different
       path) ([process.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs))
     - sticky `user_edited` field protection on update
       ([process.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs))
     - thumbnail invalidation — only when bytes actually changed
       ([process.rs:339, 411–428](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs))
     - cover-thumbnail enqueue
       ([process.rs:511 (insert path) and update path equivalent](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs))
  ...
  5. **Metadata rollup** — refresh genre/tag/credit junction tables
     for the series ([mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs)).
...
Per
[`detect_special_type`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs).
...
- **Tombstone reconcile**:
  [`reconcile_library_seen`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/reconcile.rs)
  ([called from mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs)).
  ...
  - [`mark_empty_series_removed`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/reconcile.rs)
    flips the series row to removed when the last active issue is
    gone.
- **Status reconcile**:
  [`reconcile_series_status_many`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/reconcile_status.rs)
  ([called from mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs)).
...
[`reconcile::reconcile_series`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/reconcile.rs)
...
- `enqueue_post_scan_for_library` →
  [post_scan.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/jobs/post_scan.rs) —
...
- `spawn_cbl_rematch_all` ([mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs))
  — saved-views: when the scan added/restored issues, re-resolve
  previously-missing CBL entries fire-and-forget.

[`finalize_run`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs)
...
- **Folder-level mtime check** — when `force=false` and the series row
  ...
  [`list_archives_changed_since`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/enumerate.rs)
...
- **Per-file size+mtime fingerprint** — even on a folder that *did*
  ...
  ([process.rs:147–152, 518–522](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs)).
...
- **`user_edited` stickiness on update** — fields the user has edited
  ...
  consulted at [process.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs),
...
- **Soft-delete + return lifecycle** — files missing on disk are
  ...
  ([reconcile.rs:99–164, 273–304](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/reconcile.rs)).
  When every issue in a series is removed,
  [`mark_empty_series_removed`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/reconcile.rs)
  flips the series row too.
- **Auto-confirm cron** —
  [`auto_confirm_sweep`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/reconcile.rs)
  runs daily at 04:00 UTC
  ([scheduler.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/jobs/scheduler.rs))
...
- **`series.json` re-read every scan** — there is no caching layer;
  ...
  folder ([mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs)).
...
|---|---|---|---|---|
| 1 | Folder mtime fast-path | [build_library_scan_plan mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs), [enumerate.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/enumerate.rs) | `force=false` ∧ recursive max mtime ≤ `series.last_scanned_at` → mark folder `skipped_unchanged`, call `health.touch_folder`, return `processed=false`. | `?force=true` on `POST /libraries/:slug/scan`. |
| 2 | Per-file size+mtime fingerprint | [process.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs), [mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs) | `force=false` ∧ existing row's `(file_size, file_mtime)` match disk ∧ no `comicinfo_count` backfill needed → `files_unchanged++`, `health.touch_file`, skip hash + parse. | Same `force=true` from any tier; default `true` for manual series/issue scans, `false` for file-watch jobs. |
| 3 | `comicinfo_count` backfill override | [process.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs) | When a row lacks both `comicinfo_count` and a raw `count` in `comic_info_raw`, treat the row as stale and force a re-ingest *even when* size+mtime match. | Inverts (1) — there is no bypass; the override fires automatically. |
| 4 | Thumbnail invalidation skip | [process.rs:339, 411–428](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs) | On the update path, when `row_matches_file(row, size, mtime)` is true, skip clearing `thumbnails_generated_at` and skip wiping the strip dir. | None — this is always desired. |
| 5 | Batch-rollback isolation | [mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs) | One ingest failure rolls back its `scan_batch_size` chunk (default 100) and continues; the rest of the folder still commits. | None. |
| 6 | Deferred status reconcile (full scans) | [mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs), [reconcile_status.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/reconcile_status.rs) | Library scans defer per-folder `reconcile_series_status` and run a single batch call after Phase 4 instead of N small ones. | Per-series scans run the helper inline ([mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs)). |
| 7 | Health touch-on-skip | [mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs), [process.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs) | Skipped files / folders call `health.touch_file` / `touch_folder` so the auto-resolve sweep does not close issues whose root cause is still on disk but wasn't re-emitted. | None. |
| 8 | Per-scan series identity cache | [known_series_by_folder mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs) | One `SELECT folder_path, id, last_scanned_at FROM series WHERE library_id = ?` at planning time; reused across every folder. Avoids N folder-by-folder lookups. | `force=true` still uses this cache; it only bypasses the mtime/content fast paths. |
| 9 | In-memory live-progress tracker | [LiveProgressTracker mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs), 750 ms heartbeat at [mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs) | Atomic counters in memory; `scan_runs.stats` JSON is written only on actual progress changes or every 750 ms. Prevents per-file DB churn during a scan. | None. |
| 10 | Force-rescan tiers | [api/libraries.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/api/libraries.rs), [jobs/scan_series.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/jobs/scan_series.rs), [process.rs:129–146, 164](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs) | `force` propagates from the trigger through the job into `process_planned_folder` and `ingest_one_with_fingerprint`, disabling (1), (2), and the `defer_status_reconcile` short-circuit. | The `force` param itself is the bypass. Library default `false`; manual series/issue clicks default `true`; file-watch defaults `false` (don't burn CPU on every save). |
| 11 | Move/dedupe shortcut | [process.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs) | Before `INSERT`ing a new issue row, check whether an existing row already has this content hash as its id. If the old path is missing, update the issue path and maintain `issue_paths`; if the old path still exists, emit `DuplicateContent` and `files_duplicate++`. Without this, the insert would PK-violate and roll back the entire batch. | None — the scanner always checks. |
...
[`HealthCollector`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/health.rs)
...
- Each row is keyed on `(library_id, fingerprint)`. Re-emitting the
  ...
  duplicating it (
  [`fingerprint`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/health.rs)).
- **Auto-resolve** — at `finalize`, library-wide scans set
  ...
  ([health.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/health.rs)).
...
|---|---|---|---|---|---|
| `FileAtRoot` | warning | Archive sits at the library root, not inside a series folder. | [enumerate Phase 2 → mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs) | `{ path }` | Move into a series folder. |
| `EmptyFolder` | warning | Direct child of root has no entries. | [enumerate Phase 2 → mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs) | `{ path }` | Add files or remove the folder. |
| `AmbiguousFolder` | warning | Folder violates the two-layouts contract: archives at depth-1 *and* non-allowlist archive-bearing subdirs; or no archives at depth-1 with only allowlist subdirs; or 3-deep nesting; or a stray archive directly inside a publisher folder. The subtree is skipped — better than guessing. | [enumerate Phase 2 → mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs) | `{ path, reason }` (`reason` carries an actionable hint that adapts to the violation shape) | Fix the on-disk layout per §4.2 "Two supported on-disk layouts". |
| `UnreadableFile` | error | Per-issue scan target is masked by the library's ignore globs. | [run_issue_phase mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs) | `{ path, error }` | Adjust `library.ignore_globs`, or move the file out of the ignored path. |
| `UnreadableArchive` | error | OS / archive-layer I/O error opening the archive. | [process.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs) | `{ path, error }` | Check perms, replace the file. |
| `MissingComicInfo` | info | Archive has no `ComicInfo.xml`. **Gated** on `library.report_missing_comicinfo=true` — loose libraries don't get spammed by default. | [process.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs) | `{ path }` | Tag with ComicTagger / Mylar, or flip the per-library setting off. |
| `MalformedComicInfo` | error | `ComicInfo.xml` exists but XML parse failed. | [process.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs) | `{ path, error }` | Re-tag. |
| `DuplicateContent` | warning | A new file's BLAKE3 hash matches an existing issue's id and the existing path is still present (fast-path #11). | [process.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs) | `{ path_a, path_b }` (paths sorted alphabetically — fingerprint is order-stable). | Decide which copy to keep; renamed files whose old path is gone are handled as moves. |
| `UnsupportedArchiveFormat` | warning | File extension is `.cbr` or `.cb7`. Recognized but the readers aren't shipped — see [crates/archive/src/cbr.rs](https://github.com/mbryantms/folio/blob/main/crates/archive/src/cbr.rs), [cb7.rs](https://github.com/mbryantms/folio/blob/main/crates/archive/src/cb7.rs). | [process.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs) | `{ path, ext }` | Convert to CBZ for now. |
...
([api/ws_scan_events.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/api/ws_scan_events.rs)).
...
capacity 1024
([events.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/events.rs));
laggy receivers get a `lagged` advisory frame
([ws_scan_events.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/api/ws_scan_events.rs))
and are expected to refresh manually. The schema lives in
[`ScanEvent`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/events.rs).

| Event | Where emitted | Payload | Notes |
|---|---|---|---|
| `scan.started` | [mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs), [343](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs), [408](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs) | `{ library_id, scan_id, at }` | Once per scan. UI clears prior progress, switches to "running". |
| `scan.progress` | [emit_progress mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs); also writes a `scan_runs.stats` snapshot | `{ library_id, scan_id, kind, phase, unit, completed, total, current_label, files_seen, files_added, files_updated, files_unchanged, files_skipped, files_duplicate, issues_removed, health_issues, series_scanned, series_total, series_skipped_unchanged, files_total, root_files, empty_folders, elapsed_ms?, phase_elapsed_ms?, files_per_sec?, bytes_per_sec?, active_workers?, dirty_folders?, skipped_folders?, eta_ms? }` | Cumulative counters plus optional live throughput/timing fields. Phase strings: `planning`, `planning_complete`, `scanning`, `reconciling`, `reconciled`, `enqueueing_thumbnails`, `complete`. Unit strings: `planning`, `work`, `file`. Heartbeat at 750 ms during the scanning phase only when counters changed ([mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs)). |
| `scan.series_updated` | [mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs) | `{ library_id, series_id, name }` | Fires once per series-folder enter. **Throttled** to ≤10/sec/library by the broadcaster ([events.rs:29, 158–170](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/events.rs)). UI tail shows ~8 most recent. |
| `scan.health_issue` | [health.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/health.rs) | `{ library_id, scan_id, kind, severity, path? }` | Fires on every emitted issue (not throttled — issue volume is bounded by file count). UI toasts only `error` severity by default. |
| `scan.completed` | [mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs) | `{ library_id, scan_id, added, updated, removed, duration_ms }` | Once per successful scan. UI invalidates queries (scan_runs, health, series, removed-issues). |
| `scan.failed` | [mod.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs) | `{ library_id, scan_id, error }` | Same shape as completed, terminal too. |
| `thumbs.started` | [post_scan.rs handle_thumbs](https://github.com/mbryantms/folio/blob/main/crates/server/src/jobs/post_scan.rs) | `{ library_id, issue_id, kind }` (`cover` / `page_map` / `cover_page_map`) | Post-scan worker, not the scanner — but visible on the same WS. |
...
| `lagged` | [ws_scan_events.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/api/ws_scan_events.rs) | `{ "type": "lagged", "skipped": n }` | Not part of `ScanEvent`; raw JSON. Sent when the broadcast receiver fell behind. Client may refresh. |

### What's persisted

The `scan_runs` table
([entity/scan_run.rs](https://github.com/mbryantms/folio/blob/main/crates/entity/src/scan_run.rs)) is the
...
- Scanner-to-thumbs invariant: when an issue's bytes change, the
  ...
  ([process.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs)).
- After the scan, [`enqueue_post_scan_for_library`](https://github.com/mbryantms/folio/blob/main/crates/server/src/jobs/post_scan.rs)
...
- For per-series and per-issue scans the equivalent narrowing is
  [`enqueue_post_scan_for_series`](https://github.com/mbryantms/folio/blob/main/crates/server/src/jobs/post_scan.rs).

### Search index

[`SearchJob`](https://github.com/mbryantms/folio/blob/main/crates/server/src/jobs/post_scan.rs) is available
but is not enqueued by default scans. The handler at
[post_scan.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/jobs/post_scan.rs) is
...
[`DictionaryJob`](https://github.com/mbryantms/folio/blob/main/crates/server/src/jobs/post_scan.rs) is
...
> concerns surface. See the [incompleteness audit](https://github.com/mbryantms/folio/blob/main/docs/dev/incompleteness-audit.md#d-3-dictionary-did-you-mean-trigram-refresh).

### Audit log

The scanner does not currently emit audit-log entries. Library /
issue / series CRUD goes through
[`crate::audit::record`](https://github.com/mbryantms/folio/blob/main/crates/server/src/audit/mod.rs)
from the API handlers, but scan triggers, soft-deletes from
reconcile, and auto-confirm sweeps are not audited today.

### CBL (saved views)

[`spawn_cbl_rematch_all`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/mod.rs)
...
- A returning file (same content hash, same path, file back on disk) is
  ...
  ([reconcile.rs:62–68, 198–204](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/reconcile.rs)).
...
| `comic_scan_health_issues_open` | `library_id`, `severity` | gauge ([health.rs](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/health.rs)) |
...
- **Dedupe-by-content + move-vs-duplicate semantics** (spec §6, §10.1
  ...
  - When the existing primary path still exists, the scanner emits a
    ...
    [`process.rs::ingest_one`](https://github.com/mbryantms/folio/blob/main/crates/server/src/library/scanner/process.rs).
  ...
  [crates/server/tests/scanner_smoke.rs](https://github.com/mbryantms/folio/blob/main/crates/server/tests/scanner_smoke.rs)
