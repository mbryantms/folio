...
describe("buildOps", () => {
  ...
  it("emits a Reorder permutation over survivors", () => {
    ...
    const slots: PageSlot[] = [
      { orig: 2, rotation: 0, removed: false, replaceId: null },
      { orig: 0, rotation: 0, removed: false, replaceId: null },
      { orig: 1, rotation: 0, removed: false, replaceId: null },
    ];
    expect(buildOps(slots)).toEqual([
  ...
  it("orders removes → reorder → rotate → replace for a combined edit", () => {
    ...
    const slots: PageSlot[] = [
      { orig: 3, rotation: 180, removed: false, replaceId: null },
      { orig: 0, rotation: 0, removed: false, replaceId: null },
      { orig: 2, rotation: 0, removed: false, replaceId: "img" },
      { orig: 1, rotation: 0, removed: true, replaceId: null },
    ];
    expect(buildOps(slots)).toEqual([
      { kind: "remove", ordinal: 1 },
      { kind: "reorder", new_order: [2, 0, 1] },
