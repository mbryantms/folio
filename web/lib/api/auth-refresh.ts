 ...
 * `POST /auth/refresh`, then retries the original request.
 *
...
/**
 * Fetch a path under the `/api` prefix with cookie credentials and implicit
 * 401-driven refresh. Mirrors `fetch`'s API but takes the path *without*
 * the `/api` prefix and forces `credentials: "include"`.
 */
