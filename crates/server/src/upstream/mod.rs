...
pub async fn forward(
    ...
    // X-CSP-Nonce: per-request value generated by `nonce::set_nonce`.
    // Next's middleware (`web/proxy.ts`) reads this and re-publishes
    // it as `x-nonce` on the inbound request the App-Router sees, so
    // RSC layouts can read it via `headers()` and Next auto-stamps it
    // onto every framework-emitted `<script>` tag. Strip any inbound
    // value first so a malicious client can't substitute their own.
    outbound_headers.remove("x-csp-nonce");
    if let Some(n) = nonce
        && let Ok(v) = HeaderValue::from_str(n)
    {
        outbound_headers.insert(HeaderName::from_static("x-csp-nonce"), v);
    }
