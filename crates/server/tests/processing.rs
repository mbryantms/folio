...
/// still ingests as an issue, and a `MalformedComicInfo` health row with
/// error severity lands.
#[tokio::test]
async fn malformed_comicinfo_scan_succeeds_and_surfaces_health_row() {
    let app = TestApp::spawn().await;
    let tmp = tempfile::tempdir().unwrap();
    let folder = tmp.path().join("Series Broken (2025)");
    std::fs::create_dir_all(&folder).unwrap();
    // Unclosed tag + garbage — quick-xml must reject, the scan must not.
    write_cbz_with_xml(
        &folder.join("Broken 001.cbz"),
        1,
        1,
        Some("<?xml version=\"1.0\"?><ComicInfo><Series>Broken<Number></ComicInfo"),
        None,
    );
    // A healthy sibling proves the scan carried on past the bad file.
    write_cbz_with_xml(
        &folder.join("Broken 002.cbz"),
        2,
        1,
        Some("<?xml version=\"1.0\"?><ComicInfo><Series>Broken</Series></ComicInfo>"),
        None,
    );
    ...
    // Both files exist as issues (the malformed one is tolerated).
    let issues = IssueEntity::find().all(&state.db).await.unwrap();
    assert_eq!(issues.len(), 2, "malformed file still ingests");

    let rows = HealthEntity::find().all(&state.db).await.unwrap();
    let malformed: Vec<_> = rows
        .iter()
        .filter(|r| r.kind == "MalformedComicInfo")
        .collect();
    assert_eq!(
        malformed.len(),
        1,
        "exactly one MalformedComicInfo row: {rows:?}"
    );
    assert_eq!(malformed[0].severity, "error");
    assert!(
        malformed[0].payload.to_string().contains("Broken 001"),
        "row points at the bad file: {}",
        malformed[0].payload
    );
}

/// CQ-TEST-4 (audit 2026-07): a structurally valid archive with zero image
/// entries used to ingest as a silent 0-page issue. It still ingests (the
/// file may be fixable in place), but now surfaces a `NoPages` health row.
#[tokio::test]
async fn imageless_archive_surfaces_no_pages_health_row() {
    let app = TestApp::spawn().await;
    let tmp = tempfile::tempdir().unwrap();
    let folder = tmp.path().join("Series Empty (2025)");
    std::fs::create_dir_all(&folder).unwrap();
    // Valid zip, only a text entry — zero image pages.
    {
        let f = std::fs::File::create(folder.join("Empty 001.cbz")).unwrap();
        let mut zw = zip::ZipWriter::new(f);
        let opts: zip::write::SimpleFileOptions = zip::write::SimpleFileOptions::default()
            .compression_method(zip::CompressionMethod::Stored);
        zw.start_file("readme.txt", opts).unwrap();
        zw.write_all(b"no pages here").unwrap();
        zw.finish().unwrap();
    }
    // A normal sibling so the folder ingests as a series either way.
    write_cbz_with_xml(&folder.join("Empty 002.cbz"), 1, 2, None, None);

    let lib_id = create_library(&app, tmp.path()).await;
    let state = app.state();
    scanner::scan_library(&state, lib_id)
        .await
        .expect("scan succeeds");

    let rows = HealthEntity::find().all(&state.db).await.unwrap();
    let no_pages: Vec<_> = rows.iter().filter(|r| r.kind == "NoPages").collect();
    assert_eq!(no_pages.len(), 1, "one NoPages row expected: {rows:?}");
    assert_eq!(no_pages[0].severity, "warning");
    assert!(
        no_pages[0].payload.to_string().contains("Empty 001"),
        "row points at the imageless file: {}",
        no_pages[0].payload
    );

    // And the healthy sibling did NOT get flagged.
    assert!(
        !no_pages[0].payload.to_string().contains("Empty 002"),
